...
d.slopes%>%
  ...
  theme_cowplot()+
  panel_border()+
  facet_wrap(~host,nrow = 1, scales = "free_y")+
...
library(nlme)
  ...
  mutate(counts=as.numeric(counts ))%>%
  mutate(no.ml = counts/(dilution*ml.plated))%>%
  group_by(host,phage,spore, group)%>%
  mutate(suvival.perc = no.ml/no.ml[heat.min==0])%>%
  ungroup()%>%
  mutate(uid=paste(host,phage,spore,group, sep = "."))%>%
  rename(Time=heat.min)->   d.glm
