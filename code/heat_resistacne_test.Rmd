...
```{r}
plot_grid(p.decay,p.ratio,
          labels = c("A","B"))+
  ggsave2(here("figures", "lines_and_ratio.png"),
          width = 8,height = 5)
```
...
d.slopes%>%
  ...
  pivot_longer(cols=c("ratio.cfu", "ratio.pfu"),names_to = "spore.type", values_to="slope.ratio")%>%
  mutate(phage="Goe2")-> ratio.goe2
...
d.slopes%>%
  ...
  select(-`non-infected host spore`,-`infected host spore`,-`viral spore`)%>%
  pivot_longer(cols=c("ratio.cfu", "ratio.pfu"),names_to = "spore.type", values_to="slope.ratio")%>%
    mutate(phage="SPO1")-> ratio.spo1
  

rbind(ratio.goe2,ratio.spo1)%>%
  separate(colony, into = c("host2", "group"), remove = F)%>%
  mutate(spore=ifelse(spore.type=="ratio.cfu", "host\nspore", "viral\nspore"))%>%
  ggplot(aes(x=spore, y=slope.ratio))+
  ...
  xlab("w. phage spore type")+
    theme(legend.position = "none") -> p.ratio

p.ratio+
  ggsave2(here("figures","slope_ratio.png"), width = 7, height = 7)
