...
# Plot lines of paired samples
```{r}
p.decay.colony <- 
d.rep.plot%>%
  mutate(spore=as_factor(spore))%>%
  mutate(spore=fct_relevel(spore,c("non-infected host spore","infected host spore","viral spore")))%>%
  mutate(colony=str_replace(group,"g","col"))%>%
    ggplot(aes(x=heat.min, y=perc, color=spore))+
    geom_line(aes(group=interaction(host,phage,assay,group,spore)),size=1)+
    geom_point(size=2,shape=21, fill="white")+
  scale_y_log10()+
  facet_grid(phage ~ host + colony)+
  theme_cowplot()+
  panel_border()+
  xlab("Time at 90dC (min)")+
  ylab("Survival percent (log)")+
...
p.decay.colony+
  ggsave2(here("figures", "paired_heatTreat_ALL.png"),width = 6,height = 6)
...
# d.lm.fit.log <- 
#   d.lm%>%
#    do(glance(lm(log10(perc)~heat.min,.)))
# 
# comp <- 
#   full_join(
#    select(d.lm.fit,host,phage,assay,group, lin.lm=statistic),
#    select(d.lm.fit.log,host,phage,assay,group, log.lm=statistic)
#   )
# 
# comp%>%
#   ggplot(aes(lin.lm,log.lm))+
#   geom_point()
...
d.lm <-
  d.lm%>%
   do(tidy(lm(log10(perc)~heat.min,.)))
  ...
  d.slopes%>%
    ...
    mutate(colony=str_replace(colony,"colg","col"))%>%
    rename(slope=estimate)%>%
    write_csv(here("data/HeatDecaySlopes.csv"))
...
# Plot lines of paired samples
```{r}
# arrange slope estimate data for plotting
#replicate nophage data for bith phages
d.lm.goe2 <- d.lm.estimates%>%
  filter(phage!="SPO1")%>%
      mutate(spore=case_when(assay=="PFU" ~ "viral spore",
                           phage=="noPhage" ~ "non-infected host spore",
                           TRUE ~ "infected host spore"))%>%
...
d.lm.plot <- (rbind(d.lm.goe2,d.lm.spo1))
...
d.rep.plot%>%
  ...
  mutate(colony=str_replace(group,"g","col"))%>%
    ggplot(aes(x=heat.min, y=perc, color=spore))+
    geom_line(aes(group=interaction(host,phage,assay,group,spore)),size=.3,alpha=.5)+
   geom_abline(data=d.lm.plot,
                aes(slope=slope, intercept=intercept, color=spore),size=1, linetype=2)+
    geom_point(aes(fill=spore),size=2,shape=21, alpha=.8, color="black")+
  facet_grid(phage ~ host + colony)+
  theme_cowplot()+
  panel_border()+
  xlab("Time at 90dC (min)")+
...
# 
p.slope.colony+
  ggsave2(here("figures", "paired_slopes.png"),width = 6,height = 6)
...
# I will look at difference of slopes of infected/non infected pairs
d.slopes%>%
  filter(phage!="SPO1")%>%
  # define colony variable
  mutate(colony=interaction(host, group))%>%
  #resistance  = 1+slope
  mutate(resistance=1+estimate)%>%
...
d.slopes%>%
  ...
  select(-`non-infected host spore`,-`infected host spore`,-`viral spore`)%>%
  pivot_longer(cols=c("diff.cfu", "diff.pfu"),names_to = "spore.type", values_to="slope.diff")%>%
  mutate(phage="SPO1")-> diff.spo1
  

rbind(diff.goe2,diff.spo1)%>%
  separate(colony, into = c("host2", "group"), remove = F)%>%
  mutate(spore=ifelse(spore.type=="diff.cfu", "host\nspore", "viral\nspore"))%>%
  ggplot(aes(x=spore, y=slope.diff))+
  geom_hline(yintercept = 0, linetype=2)+
  # geom_crossbar(data=filter(sum.slopes,phage!="Goe2"), aes(ymin=ymin, ymax=ymax, fill=spore),alpha=.3, position = position_dodge(width=.8), width=.5)+
  geom_jitter(aes( shape=group),size=2, stroke=2, width=.2, height = 0,alpha=0.5,show.legend = F)+
  scale_shape_manual(values = c(21,23,24))+
  theme_cowplot()+
  facet_grid(phage~host)+
  panel_border()+
  scale_fill_viridis_d()+
  ylab("resistance difference induced by\nphage infection (infected-control)")+
  xlab("infected spore type")+
    theme(legend.position = "none")+
  labs(caption = "resistance = 1+decay slope")-> p.diff

p.diff+
  ggsave2(here("figures","slope_diff.png"), width = 7, height = 7)
  
```

```{r}
left.col <- plot_grid(p.goe2,p.spo1, ncol = 1)
bottom_row <- 
  plot_grid(left.col,p.diff ,labels = c("B","C"))
plot_grid(p.slope.colony,bottom_row,
          labels = c("A",""), nrow = 2)+
  ggsave2(here("figures", "lines_and_diff.png"),
          width = 8,height = 8)
```
